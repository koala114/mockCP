    ...
    public static void main(String[] args) {
        try {
            ...
            host.addContext("/api/mybank/pay/digitalwallet/", new ContextHandler() {
                public int serve(Request req, Response resp) throws IOException {
                    System.out.println("卡号末三位是110、119时返回return_code失败、未知");
                    System.out.println("卡号倒数第四位是1、2时result_code返回1-失败2-未知，其它result_code返回0-成功");
                    System.out.println("金额amount是2、3、4、5、6、7时trade_state返回2-未支付; 3-已关闭; 4-支付失败; 5-已撤销; 6-用户支付中; 7-转入退款;其它trade_state返回1-成功");
                    ...
                    if (icbcaApi.equals("micropay")) {

                        micropayMap.put(bizConetntJson.get("out_trade_no").toString(), bizConetntJson);
                        DcepICBCA redempIcbcA = new DcepICBCA(bizConetntJson);
                        result = redempIcbcA.redempResponse();
                        //icbcaAmount = redempIcbcA.getAmount();

                    } else if (icbcaApi.equals("orderquery")) {
                        DcepICBCA reversalIcbcA;
                        String txnId = bizConetntJson.get("out_trade_no").toString();
                        System.out.println(micropayMap);
                        if (!Objects.isNull(micropayMap.get(txnId))){
                            reversalIcbcA = new DcepICBCA(micropayMap.get(txnId));
                        }
                        else
                            reversalIcbcA = new DcepICBCA(bizConetntJson);

                        result = reversalIcbcA.queryResponse();
                        resp.getHeaders().add("Content-Type", "application/json");
                        resp.send(200, result);
                    } else if (icbcaApi.equals("refundrequest")) {
                        DcepICBCA refundIcbcA = new DcepICBCA(bizConetntJson);
                        result = refundIcbcA.refundResponse();
            ...
            /* host.addContext("/api/mybank/pay/digitalwallet/micropay/V1", new ContextHandler() {
                public int serve(Request req, Response resp) throws IOException {
                    Map<String, String> params = req.getParams();
                    DcepICBCA icbcA = new DcepICBCA(params.get("biz_content"));
                    String result = icbcA.redempResponse();
            ...
            host.addContext("/api/mybank/pay/digitalwallet/orderquery/V1", new ContextHandler() {
                public int serve(Request req, Response resp) throws IOException {
                    Map<String, String> params = req.getParams();
                    DcepICBCA icbcA = new DcepICBCA(params.get("biz_content"));
                    String result = icbcA.queryResponse();
